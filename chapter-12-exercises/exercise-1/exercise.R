...
avocados %>% mutate(Date = as.Date(Date))
...
size_gathered <- by_size %>% gather(size, volume, -Date)
# Using `size_gathered`, compute the average sales volume of each size 
# (hint, first `group_by` size, then compute using `summarize`)
size_gathered <- size_gathered %>% group_by(size) %>% summarize(av_sales = mean(size_gathered$volume))
...
type_sales <- avocados %>% group_by(Date, type) %>% summarize(sum(Total.Volume))
...
# the `spread()` function!
by_type_wide <- type_sales %>% spread(type, sum(Total.Volume))
